...
(() => {
  ...
  const CALCULATOR_ID = "envelope-budget";

  function viewerHref(storyId) {
    // This script runs under /calculators/envelope-budget/
  ...
  function renderRelatedStories(relatedRefs, storiesById) {
    const host = findRelatedStoriesHost();
    if (!host) return;
  ...
  function initRelatedStories() {
    ...
    Promise.all([fetchJson(calcStoriesUrl), fetchJson(storiesUrl)])
      .then(([calcStories, storiesData]) => {
        const relatedRefs = (calcStories && calcStories[CALCULATOR_ID]) || [];
